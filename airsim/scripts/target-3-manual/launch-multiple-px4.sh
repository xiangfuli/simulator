...
$UNREAL_BLOCK_BIN -settings="$project_root_dir/airsim/settings/px4-multiple-settings-1.json" -windowed -vulkan 2>&1 > $current_directory/../logs/airsim.log &
AIRSIM_PID=$!
echo "Airsim running on process $AIRSIM_PID"

cd $PX4_DIR && make px4_sitl
PX4_SYS_AUTOSTART=10016 ./build/px4_sitl_default/bin/px4 -i 0
