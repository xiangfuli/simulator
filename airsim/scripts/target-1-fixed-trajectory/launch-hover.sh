...
cleanup() {
  ...
  sleep 0.5
  echo "Killing COMMAND_PID $COMMAND_PID"
...
python3 $current_directory/../../examples/python/command_drone_hover.py --vehicle_name "SimpleFlight0" --duration 36000 --height -6  2>&1 > $current_directory/logs/command.log &
COMMAND_PID=$!
